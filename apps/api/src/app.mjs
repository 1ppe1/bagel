...
import { anchorStatuses, artifactTypes, workflowStatuses } from '@docsync/core';
...
function isArtifactType(value) {
  return typeof value === 'string' && artifactTypes.includes(value);
}
...
function validateWorkspaceBody(body) {
  ...
  if (!Array.isArray(body.artifacts) || !Array.isArray(body.links) || !Array.isArray(body.comments)) {
    return { error: 'invalid_request', message: 'artifacts, links, and comments arrays are required.' };
  }
  ...
  for (const link of body.links) {
    if (
      ...
      return { error: 'invalid_request', message: 'Each link needs source/target referencing artifact ids.' };
    }
  }
  ...
  for (const comment of body.comments) {
    if (
      ...
      typeof comment.artifactId !== 'string' ||
      !ids.has(comment.artifactId) ||
      typeof comment.reviewerInitials !== 'string' ||
      typeof comment.reviewerName !== 'string' ||
      typeof comment.text !== 'string' ||
      typeof comment.fixInstruction !== 'boolean' ||
      typeof comment.resolved !== 'boolean'
    ) {
      return { error: 'invalid_request', message: 'Each comment needs id, artifactId, reviewer info, text, fixInstruction, resolved.' };
    }
  }
  ...
  return {
    value: {
      ...
      comments: body.comments.map((comment) => ({
        ...
        reviewerInitials: comment.reviewerInitials,
...
function bridgeStyleTag() {
  return `<style data-docsync-bridge="true">
    ...
    [data-docsync-pill="true"] {
      ...
      font: 600 13px/1 -apple-system, BlinkMacSystemFont, "Segoe UI", sans-serif !important;
      box-shadow: 0 4px 14px rgba(35, 53, 47, 0.28) !important;
      cursor: pointer !important;
      white-space: nowrap !important;
    }

    [data-docsync-pill="true"]:hover {
...
function validateCreateCommentBody(body) {
  ...
  if (body.fixInstruction !== undefined && typeof body.fixInstruction !== 'boolean') {
    return { error: 'invalid_request', message: 'fixInstruction must be a boolean.' };
  }
...
export function createApp(options = {}) {
  ...
  app.get('/api/workspace', (c) => {
    const workspace = storage.getWorkspace();
    if (!workspace) {
      return jsonError(c, 404, 'workspace_not_found', 'No workspace has been pushed yet.');
    }
  ...
  app.post('/api/workspace', async (c) => {
    ...
    const validated = validateWorkspaceBody(parsed.body);
    if (validated.error) {
      return jsonError(c, 400, validated.error, validated.message);
    }

    const result = storage.setWorkspace(validated.value);
