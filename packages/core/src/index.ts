...
export type Comment = {
  ...
  body: string;
  authorName?: string;
  /** When true, this comment is a direct instruction for the local AI to apply,
   *  not just feedback. Surfaced prominently in the generated agent context. */
...
export type ContextComment = Pick<
  Comment,
  'id' | 'body' | 'fixInstruction' | 'workflowStatus' | 'anchorStatus' | 'revisionId' | 'anchor'
> & {
...
// --- Workspace graph -------------------------------------------------------
// The workspace graph home is driven by a manifest the CLI pushes to the API.
// Colors/avatars are a UI concern derived in the web app, not stored here.

export const artifactTypes = ['spec', 'plan', 'report', 'diagram', 'notes'] as const;
...
export type PushWorkspaceRequest = WorkspaceGraph;
